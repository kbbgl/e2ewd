...
3) Open the `config.properties` file and fill it out according to the following options:
 ...
 | `requestTimeoutInSeconds`     | The number of seconds to wait until timing out of the REST API requests to retrieve ElastiCubes, Live Connections and JAQL.                                                                                                       | `int`     | `300`                                                                           |
 ...
 | (Optional) `runningRemotely` | Whether the application is running remotely and not on the server. This option overrides options `writeTestToRepository`, `runBrokerHealthCheck`, `ecDump`, `ecsDump`, `iisDump`, `restartECS`, `restartIIS` as these options require the application to run on premise.                                                                                                            | `boolean`  | `true` or `false` [default]|
 <br></br>
